...
class TestLinearSVC(unittest.TestCase):
    ...
    def test_predict_binary(self):
        ...
        y_pred = model.predict(X)
        self.assertEqual(y_pred.shape, (100,), "Prediction shape should match number of samples")
        self.assertTrue(np.all(np.isin(y_pred, [-1, 1])), "Predictions should be -1 or 1")
    ...
    def test_fit_multiclass(self):
        ...
        model = LinearSVC()
        model.fit(X, y)
        self.assertTrue(hasattr(model, 'models_'), "Multi-class models should be stored")
        self.assertEqual(len(model.models_), 3, "Should have one model per class")
        for submodel in model.models_:
            self.assertIsInstance(submodel, LinearSVC, "Submodels should be LinearSVC instances")
