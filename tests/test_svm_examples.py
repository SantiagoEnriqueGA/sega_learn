...
class TestExamplesNearestNeighbors(unittest.TestCase):
    """
...
def load_tests(loader, tests, pattern):
    ...
    example_files = glob.glob(os.path.join(os.path.dirname(__file__), '..\\examples\\svm\\*.py'))
    ...
    for example_file in example_files:
        ...
        if test_name in test_exeptions:
            setattr(TestExamplesNearestNeighbors, test_name, lambda self, example_file=example_file: TestExampleExceptions().test_main(example_file))
        else:
