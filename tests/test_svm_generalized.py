...
class TestGeneralizedSVR(unittest.TestCase):
    ...
    def test_fit_regression(self):
        ...
        self.assertEqual(model.w.shape, (20,), "Weight shape should match number of features")
    ...
    def test_predict_regression(self):
        ...
        self.assertEqual(y_pred.shape, (100,), "Prediction shape should match number of samples")
    ...
    def test_decision_function_regression(self):
        ...
        self.assertEqual(decisions.shape, (100,), "Decision function output shape should match number of samples")
        self.assertTrue(np.all(np.isfinite(decisions)), "Decision values should be finite")
        self.assertTrue(np.allclose(decisions, model.predict(X)), "Decision function should match predictions")
    ...
    def test_score_regression(self):
        ...
        self.assertTrue(-1 <= score <= 1, "R² score should typically be between -1 and 1")
    ...
    def test_is_fitted(self):
        ...
        model = GeneralizedSVR(kernel='linear')
        self.assertFalse(model.__sklearn_is_fitted__(), "Model should not be fitted before fit")
        X, y = make_regression(n_samples=100, n_features=20, random_state=42)
        model.fit(X, y)
        self.assertTrue(model.__sklearn_is_fitted__(), "Model should be fitted after fit")
    ...
    def test_epsilon(self):
        ...
        self.assertEqual(model.epsilon, 0.5, "Epsilon should match initialization value")
...
class TestGeneralizedSVC(unittest.TestCase):
    ...
    def test_fit_binary(self):
        ...
        model.fit(X, y)
        self.assertIsNotNone(model.w, "Weights should be initialized after fitting for linear kernel")
        self.assertIsNotNone(model.b, "Bias should be initialized after fitting for linear kernel")
        self.assertEqual(model.w.shape, (20,), "Weight shape should match number of features")
        self.assertTrue(np.isscalar(model.b), "Bias should be a scalar")
    ...
    def test_predict_multiclass(self):
        ...
        if hasattr(model, 'classes_'):
            self.assertTrue(np.all(np.isin(y_pred, model.classes_)), "Predictions should be valid class labels")
    ...
    def test_fit_predict_simple_binary(self):
        ...
        model = GeneralizedSVC(kernel='linear', C=1.0, max_iter=1000, learning_rate=0.01)
        model.fit(X, y)
        y_pred = model.predict(X)
        self.assertTrue(np.all(y_pred == y), "Predictions should match true labels")
        decisions = model.decision_function(X)
        self.assertTrue(np.all(decisions * y > 0), "Decision values should have correct sign")
