...
class TestOneClassSVM(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.model = OneClassSVM(kernel='linear', max_iter=200, learning_rate=0.05, C=0.5)
    ...
    def test_fit(self):
        ...
        self.assertTrue(hasattr(self.model, 'support_vectors_'),
                        "Model should have support_vectors_ after fit")
        self.assertGreater(len(self.model.support_vectors_), 0,
                           "There should be at least one support vector")
        self.assertTrue(hasattr(self.model, 'support_vector_alphas_'),
                        "Model should have support_vector_alphas_ after fit")
        self.assertTrue(hasattr(self.model, 'b'),
                        "Model should have bias (b) set after fit")
    ...
    def test_decision_function(self):
        ...
        self.assertEqual(decisions.shape, (X.shape[0],),
    ...
    def test_predict(self):
        ...
        self.assertEqual(predictions.shape, (X.shape[0],),
                         "Predict should return an array with one prediction per sample")
        unique_preds = np.unique(predictions)
        for pred in unique_preds:
            self.assertIn(pred, [1, -1],
                          "Predictions should be either 1 (inlier) or -1 (outlier)")
    ...
    def test_is_fitted(self):
        ...
        self.assertIsNone(model.support_vectors_,
