...
class JITFlattenLayer:
    ...
    def backward(self, dA, reg_lambda=0):
        ...
        result = np.zeros((batch_size, self.input_shape[0], self.input_shape[1], self.input_shape[2]), dtype=np.float64)
...
class JITConvLayer:
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, activation="relu"):
        ...
        self.weight_gradients = np.zeros((out_channels, in_channels, kernel_size, kernel_size))
    ...
    def forward(self, X):
        ...
        if self.padding > 0:
            X_padded = np.zeros((batch_size, self.in_channels, h_in + 2 * self.padding, w_in + 2 * self.padding), dtype=np.float64)
            # Manual padding
            for b in range(batch_size):
        ...
        # Create output array
        output = np.zeros((batch_size, self.out_channels, h_out * w_out), dtype=np.float64)
                    ...
                    for c in range(self.in_channels):
                        for kh in range(self.kernel_size):
                            for kw in range(self.kernel_size):
                                ...
                                val += self.weights[o, c, kh, kw] * X_cols[b, filter_idx, i]
                    output[b, o, i] = val
        
        # Reshape output to feature map format
        output_reshaped = np.zeros((batch_size, self.out_channels, h_out, w_out), dtype=np.float64)
        for b in range(batch_size):
    ...
    def backward(self, d_out, reg_lambda=0):
        ...
        d_out_reshaped = np.zeros((batch_size, self.out_channels, self.h_out * self.w_out), dtype=np.float64)
        for b in range(batch_size):
            for c in range(self.out_channels):
        ...
        for b in range(batch_size):
            ...
            for i in range(self.h_out * self.w_out):
                for filter_idx in range(self.in_channels * self.kernel_size * self.kernel_size):
                    val = 0.0
                    for o in range(self.out_channels):
                        val += d_out_reshaped[b, o, i] * self.weights[o, filter_idx // (self.kernel_size * self.kernel_size), 
        ...
        if reg_lambda > 0:
            for o in range(self.out_channels):
                for c in range(self.in_channels):
                    for kh in range(self.kernel_size):
                        for kw in range(self.kernel_size):
                            d_weights[o, c, kh, kw] += reg_lambda * self.weights[o, c, kh, kw]
