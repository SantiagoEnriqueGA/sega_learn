class TrainingAnimator:
    ...
    def initialize(self, metrics_to_track, has_validation=False):
        ...
        if has_validation:
            self.metrics.update({f"val_{metric}": [] for metric in metrics_to_track if metric != "learning_rate"})
        ...
        for i, metric in enumerate(metrics_to_track):
            ...
            if has_validation and metric != "learning_rate":
                self.lines[f"val_{metric}"], = ax.plot([], [], 'r-', label=f'Val {metric}')
    ...
    def animate_training_metrics(self, interval=200, blit=True, save_path=None, save_format='mp4', fps=10, dpi=300):
        """
        ...
        # Set axis limits
        for i, metric in enumerate([m for m in self.metrics.keys() if not m.startswith("val_")]):
            row, col = i // 3, i % 3
            ax = self.axes[row, col]
        ...
        try:
            ...
            try:
                ...
                self.writer = FFMpegWriter(
                    fps=fps, 
                    metadata=dict(title='Model Training Progress', artist='TrainingAnimator'),
                    bitrate=1800
                )
            ...
            print(f"Animation writer successfully set up. Output will be saved to: {filepath}")
        except Exception as e:
            self.writer = None
            print(f"Error setting up animation writer: {str(e)}")
            print("Animation capture disabled. Training will continue without animation.")
            raise
