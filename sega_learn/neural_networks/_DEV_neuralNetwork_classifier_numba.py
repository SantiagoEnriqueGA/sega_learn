...
# nn = NumbaBackendNeuralNetwork([input_size] + layers + [output_size], dropout_rate=dropout, reg_lambda=reg_lambda, activations=activations)

layers = [
        JITLayer(input_size, layers[0], activation="relu"),
        JITLayer(layers[0], layers[1], activation="relu"),
        JITLayer(layers[1], layers[2], activation="relu"),
        JITLayer(layers[2], output_size, activation="softmax"),
]
nn = NumbaBackendNeuralNetwork(layers=layers, dropout_rate=dropout, reg_lambda=reg_lambda)
