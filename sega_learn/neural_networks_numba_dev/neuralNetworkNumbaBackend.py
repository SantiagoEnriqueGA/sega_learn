...
class NumbaBackendNeuralNetwork(NeuralNetworkBase):
    def __init__(self, layers, dropout_rate=0.2, reg_lambda=0.01, activations=None, compile_numba=True, progress_bar=True):
        ...
        self.trainable_layers = [layer for layer in self.layers if any(hasattr(layer, attr) for attr in ['dense_weights', 'dense_biases'])]
    ...
    def store_init_layers(self):
        ...
        self._weights = [layer.dense_weights.copy() for layer in self.layers if hasattr(layer, 'dense_weights')]
        self._biases = [layer.dense_biases.copy() for layer in self.layers if hasattr(layer, 'dense_biases')]
        
        self.weights = [layer.dense_weights.copy() for layer in self.layers if hasattr(layer, 'dense_weights')]
        self.biases = [layer.dense_biases.copy() for layer in self.layers if hasattr(layer, 'dense_biases')]
    ...
    def initialize_new_layers(self):
        ...
        for i in range(len(self.layer_sizes) - 1):
            self.layers.append(JITLayer("dense", self.layer_sizes[i], self.layer_sizes[i+1], self.activations[i]))
    ...
    def calculate_loss(self, X, y):
        ...
        weights = [layer.dense_weights for layer in self.trainable_layers if hasattr(layer, 'dense_weights')]
