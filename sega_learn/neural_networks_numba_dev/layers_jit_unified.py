...
class JITLayer:
    def __init__(self, layer_type, input_size, output_size, activation="relu",
        ...
        if layer_type == "dense":
            ...
            self.conv_weight_grad = np.zeros((output_size, input_size, kernel_size, kernel_size))
    ...
    def backward(self, dA, reg_lambda):
        if self.layer_type == "dense":
            ...
            pass
            # return self._backward_conv(dA, reg_lambda)
        elif self.layer_type == "flatten":
            pass
    ...
    def _forward_conv(self, X):
        ...
        if self.padding > 0:
            X_padded = np.zeros((batch_size, self.input_size, h_in + 2 * self.padding, w_in + 2 * self.padding), dtype=np.float64)
            # Manual padding
        ...
        # Create output array
        output = np.zeros((batch_size, self.output_size, h_out * w_out), dtype=np.float64)
        ...
        output_reshaped = np.zeros((batch_size, self.output_size, h_out, w_out), dtype=np.float64)
        for b in range(batch_size):
    ...
    def _backward_conv(self, d_out, reg_lambda=0.0):
        ...
        d_out_reshaped = np.zeros((batch_size, self.output_size, self.h_out * self.w_out), dtype=np.float64)
        for b in range(batch_size):
            for c in range(self.output_size):
        ...
        for b in range(batch_size):
            ...
            # Input gradients
            for i in range(self.h_out * self.w_out):
                for filter_idx in range(self.input_size * self.kernel_size * self.kernel_size):
                    val = 0.0
                    for o in range(self.output_size):
                        val += d_out_reshaped[b, o, i] * self.conv_weights[o, filter_idx // (self.kernel_size * self.kernel_size), 
                                                                   (filter_idx % (self.kernel_size * self.kernel_size)) // self.kernel_size,
                                                                   (filter_idx % (self.kernel_size * self.kernel_size)) % self.kernel_size]
    ...
    def _forward_flatten(self, X):
        ...
        self.flatten_input_shape = (int32(X.shape[1]), int32(X.shape[2]), int32(X.shape[3]))
        
        # Calculate flattened vector size
        self.input_size = X.shape[1] * X.shape[2] * X.shape[3]
        self.output_size = self.input_size  # For flatten layer, input and output sizes are the same
    ...
    def _backward_flatten(self, dA):
        ...
        output = np.zeros((batch_size, self.flatten_input_shape[0], self.flatten_input_shape[1], self.flatten_input_shape[2]), dtype=np.float64)
    ...
    def _im2col(self, x, h_out, w_out):
        ...
        col = np.zeros((batch_size, channels * self.kernel_size * self.kernel_size, h_out * w_out), dtype=np.float64)
    ...
    def _col2im(self, dcol, x_shape):
        ...
        dx_padded = np.zeros((batch_size, channels, h_padded, w_padded), dtype=np.float64)
