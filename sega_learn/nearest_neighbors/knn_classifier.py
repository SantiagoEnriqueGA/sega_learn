...
class KNeighborsClassifier(KNeighborsBase):
    ...
    def predict(self, X):
        ...
        if self.numba:
            ...
            predictions = _numba_predict_classifier(distances, self.y_train, self.n_neighbors)
        else:
            ...
            predictions = np.array([np.bincount(labels.astype(int)).argmax() for labels in top_k_y])
