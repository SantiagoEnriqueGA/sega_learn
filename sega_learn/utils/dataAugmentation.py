...
class SMOTE:
    ...
    def fit_resample(self, X, y, force_equal=False):
        ...
        synthetic_samples = []
        target_samples = len(X_majority) - len(X_minority) if force_equal else len(X_minority)
...
class RandomOverSampler:
    ...
    def fit_resample(self, X, y):
        ...
        indices = np.random.choice(len(X_minority), size=n_samples_to_generate, replace=True)
...
class RandomUnderSampler:
    ...
    def fit_resample(self, X, y):
        ...
        indices = np.random.choice(len(X_majority), size=n_samples_to_remove, replace=False)
        ...
        y_resampled = np.hstack([np.full(len(X_majority_resampled), majority_class), np.full(len(X_minority), minority_class)])
...
class Augmenter:
    ...
    def augment(self, X, y):
        if self.verbose: print("Starting data augmentation...")
                    
        for technique in self.techniques:
            # If classes are not balanced, apply the augmentation technique
            balance = _Utils.check_class_balance(y)
            if balance[1][0] == balance[1][1]:
                if self.verbose: print(f"   !!!!Classes are already balanced. Skipping {technique.__class__.__name__}")
                continue
            
            if self.verbose: print(f"   ...Applying {technique.__class__.__name__}")            
            X, y = technique.fit_resample(X, y)
            
        if self.verbose: print("Data augmentation completed.")
        return X, y
