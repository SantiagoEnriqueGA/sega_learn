...
def one_hot_encode(X, cols=None):
        ...
        X = pd.concat([X.drop(X.columns[cols], axis=1)] + new_columns, axis=1)  # Drop the original columns and add new columns

        if not is_dataframe:
            return X.values  # Convert back to numpy array if it was originally a numpy array
