...
def plot_ellipse(mean, cov, color, ax, label):
    ...
    v, w = linalg.eigh(cov)         # Compute eigenvalues and eigenvectors of the covariance matrix
    u = w[0] / linalg.norm(w[0])    # Normalize the eigenvector
    angle = np.arctan(u[1] / u[0])  # Compute the angle of the ellipse
    angle = 180 * angle / np.pi     # Convert to degrees
    
    ell = mpl.patches.Ellipse(xy=mean, width=2 * np.sqrt(np.abs(v[0])), height=2 * np.sqrt(np.abs(v[1])), 
                              angle=180 + angle, facecolor=color, edgecolor='black', label=label)
