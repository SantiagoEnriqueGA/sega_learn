...
initial_window = 1
wma_model_init = WeightedMovingAverage(window=initial_window)
fitted_values_init = wma_model_init.fit(train_series)
forecasted_values_init = wma_model_init.forecast(steps=forecast_steps)
...
ax.set_ylabel("Value")
title = ax.set_title(f"Weighted Moving Average Forecast (Window={initial_window})")
...
def update(window):
    ...
    des_model = WeightedMovingAverage(window=window)
    ...
    # Update the title
    title.set_text(f"Weighted Moving Average Forecast (Window={window})")
...
animation_filename = os.path.join(
    "examples/time_series/plots/mvg_wma_forecast_animation.gif"
)
# Use pillow for GIF
