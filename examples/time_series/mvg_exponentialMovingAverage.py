...
for i, alpha in enumerate(alpha_values, start=1):
    # Initialize and fit the Exponential Moving Average model
    ses_model = ExponentialMovingAverage(alpha=alpha)
    fitted_values = ses_model.fit(train_series)

    # Forecast future values using the Exponential Moving Average model
    forecast_steps = len(test_series)
    forecasted_values_ses = ses_model.forecast(steps=forecast_steps)

    # Evaluate the Exponential Moving Average model
    mse_ses = mean_squared_error(test_series, forecasted_values_ses)
    print(
        f"Alpha {alpha}: Exponential Moving Average Mean Squared Error: {mse_ses:.4f}"
    )
    ...
    plt.title(f"Exponential Moving Average Forecast (Alpha={alpha})")
    plt.legend()

plt.tight_layout()
# plt.show()
plt.savefig("examples/time_series/plots/mvg_ema_forecast.png", dpi=300)
