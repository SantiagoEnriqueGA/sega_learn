...
class TestTime:
    ...
    def measure_performance(self, transform_class_sklearn, transform_class_sega, X):
        def transform_performance(transform_class):
            ...
            if (
                transform_class == PolynomialTransform
                or transform_class == PolynomialFeatures
            ):
        ...
        sklearn_times = [
            transform_performance(transform_class_sklearn) for _ in range(self.num_runs)
        ]
        sega_times = [
            transform_performance(transform_class_sega) for _ in range(self.num_runs)
        ]
...
def plot_results(results, title):
    ...
    plt.grid(True)
    plt.savefig(
        f"tests_performance/scalability/plots/utils_{title.replace(' ', '_')}.png"
    )
    # plt.show()
...
def test_compare_libraries(num_runs):
    ...
    for sample_size in NUM_SAMPLES_LIST:
        X, _ = synthetic_data_regression(
            n_samples=sample_size, n_features=5, noise=0.1, random_state=42
        )
        timer = TestTime(num_runs)
        avg_sklearn_time, std_sklearn_time, avg_sega_time, std_sega_time = (
            timer.measure_performance(
                transform_class_sklearn=PolynomialFeatures,
                transform_class_sega=PolynomialTransform,
                X=X,
            )
        )
