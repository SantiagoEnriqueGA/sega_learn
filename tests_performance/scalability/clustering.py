...
def plot_results(results, title):
    ...
    plt.tight_layout()
    plt.grid(True)
    plt.savefig(f"tests_performance/scalability/plots/clustering_{title.replace(' ', '_')}.png")
    # plt.show()
...
def test_compare_libraries(models_to_test, num_runs):
    for models in models_to_test:
        ...
        for sample_size in sample_sizes:
            ...
            X, y = make_blobs(n_samples=sample_size, n_features=5, centers=NUM_K, cluster_std=0.60, random_state=1)
