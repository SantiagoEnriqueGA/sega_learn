...
class TestTime:
    ...
    def measure_performance(
        self, model_class_sklearn, model_class_sega, neighbors_to_test
    ):
        def model_performance(model_class):
            if (
                model_class == KNeighborsRegressor
                or model_class == KNeighborsClassifier
            ):
...
def test_compare_libraries(models_to_test, neighbors_to_test, num_runs):
    for models in models_to_test:
        ...
        plot_results(
            results,
            f"{sega_model.__name__} Scalability for {neighbors_to_test} Neighbors",
        )
...
def plot_results(results, title):
    ...
    sklearn_colors = sns.color_palette(
        "Blues", n_colors=len(results["N Neighbors"].unique())
    )
    sega_colors = sns.color_palette(
        "Oranges", n_colors=len(results["N Neighbors"].unique())
    )
    ...
    plt.grid(True)
    plt.savefig(
        f"tests_performance/scalability/plots/nearest_neighbors_{title.replace(' ', '_')}.png"
    )
