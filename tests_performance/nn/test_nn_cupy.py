...
def time_nn_optimizer_cupy(num_repeats=5, layer_sizes_multiplier=25):
    # Optimizer Timing
    # ---------------------------------------------------------------------------------------------
    NUM_REPEATS = num_repeats
    LAYER_SIZES_MULTIPLIER = layer_sizes_multiplier
...
def combine_timing_results_cupy(nn_times, optimizer_times, loss_times):
    ...
    output_filename = "tests_performance/nn/nn_timing_results_cupy.csv"
    save_timing_results(output_filename, nn_times, optimizer_times, loss_times)


def time_nn_epoch_cupy(num_repeats=5, layer_sizes_multiplier=1, dataset_sizes=[1_000, 5_000, 10_000, 50_000, 100_000], save_csv=True):
    ...
    if save_csv:
        ...
        with open("tests_performance/nn/nn_timing_results_cupy_train.csv", mode='w', newline='') as file:
            writer = csv.writer(file)
...
if __name__ == "__main__":
    # combine_timing_results_cupy(time_nn_cupy(), time_nn_optimizer_cupy(), time_nn_loss_cupy())
    time_nn_cupy(num_repeats=5)
    time_nn_optimizer_cupy(num_repeats=5)
    time_nn_loss_cupy(num_repeats=5)
    time_nn_epoch_cupy(num_repeats=5, save_csv=False)