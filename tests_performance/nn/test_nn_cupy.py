...
def time_nn_optimizer_cupy(num_repeats=5, layer_sizes_multiplier=25):
    ...
    for name, optimizer in optimizers.items():
        ...
        update_avg, update_stddev, _ = time_function(
            optimizer.update_layers, NUM_REPEATS, nn.layers, dW, db
        )
...
def time_nn_loss_cupy(num_repeats=5, layer_sizes_multiplier=10, dataset_size=1_000_000):
    ...
    for name, loss_fn in loss_functions.items():
        if name == "CrossEntropyLoss":
            ...
            targets = cp.eye(layer_sizes[-1])[
                cp.random.choice(layer_sizes[-1], DATASET_SIZE)
            ]
...
def time_nn_epoch_cupy(
    ...
    if save_csv:
        ...
        with open(
            "tests_performance/nn/nn_timing_results_cupy_train.csv",
            mode="w",
            newline="",
        ) as file:
            writer = csv.writer(file)
