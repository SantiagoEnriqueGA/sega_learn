...
def init_compile():
    ...
    print("   ...Compiling small neural network", end="", flush=True)
    layer_sizes = [1, 1, 1]
    nn = NeuralNetwork(
        layers=layer_sizes,
        dropout_rate=0.5,
    ...
    print("   ...Compiling optimizers", end="", flush=True)
    ...
    print("   ...Compiling loss functions", end="", flush=True)
    ...
    print("   ...Compiling training step", end="", flush=True)
    optimizer = AdamOptimizer(learning_rate=0.01)
    scheduler = lr_scheduler_step(optimizer, lr_decay=0.1, lr_decay_epoch=10)
