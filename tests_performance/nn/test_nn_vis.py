...
plt.figure(figsize=(14, 8))
speedup_bars = sns.barplot(
    x="Function", y="Speedup (%)", data=pivot_results, palette=["#50C878"]
)
...
for i, v in enumerate(pivot_results["Speedup (%)"]):
    if v < 0:
        speedup_bars.patches[i].set_facecolor(
            "#FF7F7F"
        )  # Light red for negative speedup
