...
def kmeans_sega():
    ...
    for sample_size in sample_sizes:
        ...
        print(
            f"sega_kmeans sample_size: {sample_size:<-7} avg_time: {avg_sega_time:.4f} std_time: {std_sega_time:.4f}"
        )
...
def dbscan_sega():
    ...
    for sample_size in sample_sizes:
        ...
        print(
            f"sega_dbscan sample_size: {sample_size:<-7} avg_time: {avg_sega_time:.4f} std_time: {std_sega_time:.4f}"
        )
        results.loc[len(results)] = [sample_size, avg_sega_time, std_sega_time]
...
if __name__ == "__main__":
    ...
    results.to_csv(
        f"tests_performance/CPython_vs_PyPy/logs_clustering/kmeans_{python_executable}.csv",
        index=False,
    )
    ...
    results["version"] = python_executable
    results.to_csv(
        f"tests_performance/CPython_vs_PyPy/logs_clustering/dbscan_{python_executable}.csv",
        index=False,
    )
