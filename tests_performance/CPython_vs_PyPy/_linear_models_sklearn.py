...
def test_ols(X, y, X_test, NUM_SAMPLES, NUM_RUNS):
    ...
    avg_sklearn_time, std_sklearn_time = timer.measure_performance(
    ...
    print(
        f"\tsklearn Linear Regression: {avg_sklearn_time:10.4f} ± {std_sklearn_time:7.4f} seconds"
    )
...
def test_ridge(X, y, X_test, NUM_SAMPLES, NUM_RUNS):
    ...
    print(
        f"\tsklearn Ridge Regression: {avg_sklearn_time:10.4f} ± {std_sklearn_time:7.4f} seconds"
    )
...
def test_lasso(X, y, X_test, NUM_SAMPLES, NUM_RUNS):
    ...
    print(
        f"\tsklearn Lasso Regression: {avg_sklearn_time:10.4f} ± {std_sklearn_time:7.4f} seconds"
    )
...
def test_bayesian(X, y, X_test, NUM_SAMPLES, NUM_RUNS):
    ...
    print(
        f"\tsklearn Bayesian Ridge Regression: {avg_sklearn_time:10.4f} ± {std_sklearn_time:7.4f} seconds"
    )
...
def test_ransac(X, y, X_test, NUM_SAMPLES, NUM_RUNS):
    ...
    print(
        f"\tsklearn RANSAC Regression: {avg_sklearn_time:10.4f} ± {std_sklearn_time:7.4f} seconds"
    )
...
def test_passiveAggressiveRegressor(X, y, X_test, NUM_SAMPLES, NUM_RUNS):
    ...
    print(
        f"\tsklearn Passive Aggressive Regression: {avg_sklearn_time:10.4f} ± {std_sklearn_time:7.4f} seconds"
    )
...
def main():
    ...
    results.to_csv(
        "tests_performance/CPython_vs_PyPy/logs_linear_models/linear_models_sklearn.csv",
        index=False,
    )
