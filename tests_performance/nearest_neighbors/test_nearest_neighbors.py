...
def time_knn_classifiers(num_repeats=10, n_neighbors=list, n_records=list, n_jobs=1):
    from sklearn.neighbors import KNeighborsClassifier as sklearn_knn
    
    knn_classifiers = []
    for n in n_neighbors:
        knn_classifiers.append(KNeighborsClassifier(n_neighbors=n))
        knn_classifiers.append(sklearn_knn(n_neighbors=n, n_jobs=n_jobs))
    ...
    for n in n_neighbors:
        for n_record in n_records:
            ...
            X, y = make_classification(n_samples=n_record, n_features=20, n_classes=2)
            
            for knn_classifier in knn_classifiers:
                avg_time, stddev_time, _ = time_function(knn_classifier.fit, num_repeats, X, y)
                
                # Append the results to the DataFrame
                results.loc[len(results)] = ["sega_learn" if isinstance(knn_classifier, sega_learn.nearest_neighbors.KNeighborsClassifier) else "sklearn",
                                            n, n_record, avg_time, stddev_time]
                
    return results
    

def time_knn_regressors(num_repeats=10, n_neighbors=list, n_records=list, n_jobs=1):
    from sklearn.neighbors import KNeighborsRegressor as sklearn_knn
    
    knn_regressors = []
    for n in n_neighbors:
        knn_regressors.append(KNeighborsRegressor(n_neighbors=n))
        knn_regressors.append(sklearn_knn(n_neighbors=n, n_jobs=n_jobs))
    ...
    for n in n_neighbors:
        for n_record in n_records:
            # Generate synthetic data
            X, y = make_regression(n_samples=n_record, n_features=20)
...
def plot_results(results, title):
    ...
    sns.lineplot(x="N_Records", y="Time (s)", hue="Module", style="N_Neighbors", markers=True, dashes=False, data=results)
    plt.xscale("log")
    plt.title(title)
...
def run():
    ...
    n_records = [1_000, 5_000, 10_000, 50_000, 100_000, 500_000, 1_000_000, 5_000_000]
    num_repeats = 5
    n_jobs = 1
    
    # Run the tests
    print("Testing KNN Classifiers...")
    class_times = time_knn_classifiers(num_repeats=num_repeats, n_neighbors=n_neighbors, n_records=n_records, n_jobs=n_jobs)
    plot_results(class_times, "KNN Classifier Performance")
    
    print("\nTesting KNN Regressors...")
    reg_times = time_knn_regressors(num_repeats=num_repeats, n_neighbors=n_neighbors, n_records=n_records, n_jobs=n_jobs)
