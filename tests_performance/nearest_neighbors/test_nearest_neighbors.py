...
def time_knn_classifiers(
    ...
    results = pd.DataFrame(
        columns=["Module", "N_Neighbors", "N_Records", "Time (s)", "StdDev (s)"],
        dtype=object,
    )
        ...
        for n in n_neighbors:
            for n_record in n_records:
                ...
                X, y = make_classification(
                    n_samples=n_record, n_features=num_features, n_classes=num_classes
                )
                ...
                for knn_classifier in knn_classifiers:
                    ...
                    avg_time, stddev_time, _ = time_function(
                        knn_classifier.fit, num_repeats, X, y
                    )

                    # Append the results to the DataFrame
                    results.loc[len(results)] = [
                        "sega_learn"
                        if isinstance(knn_classifier, KNeighborsClassifier)
                        else "sklearn",
                        n,
                        n_record,
                        avg_time,
...
def time_knn_regressors(
    num_repeats=10, n_neighbors=list, n_records=list, n_jobs=int, num_features=int
):
        ...
        for n in n_neighbors:
            for n_record in n_records:
                ...
                for knn_regressor in knn_regressors:
                    ...
                    avg_time, stddev_time, _ = time_function(
                        knn_regressor.fit, num_repeats, X, y
                    )

                    # Append the results to the DataFrame
                    results.loc[len(results)] = [
                        "sega_learn"
                        if isinstance(knn_regressor, KNeighborsRegressor)
                        else "sklearn",
                        n,
                        n_record,
                        avg_time,
...
def plot_results(results, title):
    ...
    grouped_results = (
        results.groupby(["Module", "N_Neighbors", "N_Records"])
        .agg({"Time (s)": "mean", "StdDev (s)": "mean"})
        .reset_index()
    )
    ...
    for module in grouped_results["Module"].unique():
        for n_neighbors in grouped_results["N_Neighbors"].unique():
            subset = grouped_results[
                (grouped_results["Module"] == module)
                & (grouped_results["N_Neighbors"] == n_neighbors)
            ]
            plt.fill_between(
                subset["N_Records"],
                subset["Time (s)"] - subset["StdDev (s)"],
                subset["Time (s)"] + subset["StdDev (s)"],
                alpha=0.2,
            )
...
def run():
    ...
    class_times = time_knn_classifiers(
